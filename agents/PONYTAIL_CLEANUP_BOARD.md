...
- [ ] **Discord token helper:** extract shared `_discord_token()` from `list-discord-category-channels.py` + `configure-hermes-discord-hunter.sh` (+ euro/nyc configure scripts) into `scripts/linuxbox/discord_token.py` (stdlib + dotenv); callers import it — no behavior change. **Daily deslop dd-02.**
...
- [ ] **agent-pod-scheduler.sh:** add explicit prompt branch for `ponytail-cleanup` pod (ponytail rules + board path + Goal/Feature/Keep/Verify) instead of generic task_spec line
