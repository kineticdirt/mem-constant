...
| 2026-08-09 | pins look “moved” after refresh | clearing localStorage `coord_overrides` snaps to server vibes coords; pins∉GM polys for several cities | pin-lock docs; do not auto-rewrite borders; GM picks A/B/C | `PIN-COORDS-LOCK.md`, ledger `tableslop-pins-borders-drift` |
...
- Pin∉border for Paradise/Porto/Jacked — **no automated PIP fail in smoke yet** (add only with GM OK on which SoT wins).
