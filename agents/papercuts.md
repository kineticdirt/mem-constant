...
- **Status:** fixed 2026-08-10 (symptom) — potato PASS; no pin moves. **Prevention (dd-16):** `verify-runtime-state.sh` on `bundle-apply` pin-freeze FAIL sleeps 3s and re-checks once before FAIL/inbox; never auto-`--accept`. Holder `daily-deslop-dd-16`.
