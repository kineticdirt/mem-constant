...
async def cmd_export_category(
    ...
    *,
    ...
    skip_ids = exclude_channel_ids or set()
    filtered: list[discord.TextChannel] = []
    for c in text_channels:
        if c.id in skip_ids:
            print(f"Skip #{c.name} ({c.id}): excluded by channel id", flush=True)
            continue
        lname = c.name.lower()
        if any(sub.lower() in lname for sub in exclude_name_substrings if sub):
            print(f"Skip #{c.name} ({c.id}): name matches exclude substring", flush=True)
            continue
        filtered.append(c)
    text_channels = filtered
    ...
    async def on_ready() -> None:
        try:
            if args.list:
                ...
                exclude_ids: set[int] = set(args.exclude_channels or [])
                await cmd_export_category(
                    ...
                    exclude_name_substrings=tuple(args.exclude_name or ()),
                )
            elif args.channels:
                skip = set(args.exclude_channels or [])
                ch_ids = [c for c in args.channels if c not in skip]
...
def main() -> None:
    ...
    p.add_argument(
        ...
        type=int,
        metavar="ID",
        help="Channel ids to skip (with --guild/--category or --channels).",
    )
    p.add_argument(
        "--exclude-name",
        nargs="+",
        metavar="SUBSTR",
        help="Skip channels whose name contains any substring (case-insensitive), e.g. fallout.",
    )
    args = p.parse_args()
