...
- **[map layer]** — `map/layers.json` → `highways` source `map/highways.json`. Routes are **art-traced** from black road markers on `master-enhanced.png` (`scripts/tableslop/extract-art-highways.py`). Green map lines remain the visual SoT; overlay follows those markers. Prior yellow sketches live under `proposals[]`. **Do not wipe `regions-ui.json`.**
