...
| Label | Meaning |
|-------|---------|
| **Decided** | Already true in this workspace / prior signed-off charter (scope, honesty about demos, R&D posture) |
...
### Why this matters here (without overselling the stack)

This workspace already struggles productively with **resource governance**: search tokens, memory, free vs paid models, and correctness (`.cursor/rules/resource-governance.mdc`). linuxbox Hermes routes **free-first** inference with paid fallbacks; OpenRouter budgets are constrained. That experience is evidence of the *need*—metered, accountable compute with clear economics—not proof that Infranet already exists as a network. A homelab can **prototype** settlement and LLM job metering; it is not a global marketplace.

### What we already started (repo evidence)

| Prior material | What it covers | Status |
|----------------|----------------|--------|
| `projects/infranet/PLANNING.md` / `README.md` | Decentralized identity, black-box verification (FHE/MPC/ZKP sketch), NFC/mobile | Earlier architecture notes |
| `projects/infranet/COMPUTE_REWARDS.md` | Compute marketplace, resource ads, verification rewards, reputation, tokenomics for *network-internal* work | **Closest ancestor** to this proposal |
| `projects/infranet/BLOCKCHAIN_PLATFORM.md` | Programmable substrate (IVM / InfranetScript sketch) aimed at identity, storage, verification | Sketch |
| `projects/infranet/COST_ANALYSIS.md` | Gas/compute-unit cost models for crypto ops | Demo pricing ≠ market GPU price |
| Python/Rust demos & tests | In-memory blockchain, identity registry, marketplace lifecycle | **Placeholders**, not real FHE |
| Potato `BASELINE.md` (SSH-verified 2026-07-12) | Source-trace of what the demo actually does | Confirms no live crypto |
...
**Decided** near-term honesty: ship the smallest settlement loop that moves a balance when a real or mocked inference job completes. Expand verticals only when the settlement pattern holds. Per `AGENT-CHARTER.md`, this remains R&D until explicitly promoted—not accidental production.
...
## Pillar 1 — Crypto-defined compute tokens

*How cryptography defines the unit of account for compute.*
...
### What cryptography is for (plain language)

| Job | Why crypto / ledger |
|-----|---------------------|
| **Minting rules** | Who can create tokens, under what proof |
| **Ownership** | Who can spend which balance (wallets / accounts) |
...
### Issuance, mint, burn (**Proposed**)

Two issuance modes can coexist; Phase 1 should pick one primary story:
...
**Proposed MVP:** one fungible work token + simple stake locks. Split classes only when abuse forces it.
...
`COMPUTE_REWARDS.md` formulas remain useful for *network-internal* verification. Marketplace jobs use **escrowed consumer payment** as the main economic signal. `COST_ANALYSIS.md` tracks engineering/gas cost of crypto ops; keep that separate from *market* prices of GPU time.

### Wallets and accounts (**Proposed**)

- Each participant holds a **ledger account** (keypair-backed wallet) for work-token balances and escrow locks.
- Services may hold **treasury / payout** accounts that receive user payments and re-escrow into producer jobs.
- UX may hide chain jargon (“compute credits”) while the ledger still records cryptographic ownership.
- **Decided:** no secrets, private keys, or seed phrases in repo docs or git.

### Optional cash bridges (**Proposed**)
...
## Pillar 2 — Trading compute tokens without mandatory cash

*Consumers and producers settle in tokens; cash is optional.*

### Happy path (**Example** flow; pattern is **Proposed**)
...
Same spirit as `COMPUTE_REWARDS.md`’s marketplace, generalized from “verification nodes” to **any attested work class**, with LLM inference as the first high-demand class.
...
### Peer exchange without fiat (**Proposed**)

- **Work ↔ work:** tokens earned on one job class spend on another (GPU overnight → agent runs daytime).
- **Direct OTC:** two parties transfer tokens for a bilateral job without a public auction; ledger still records escrow/settle for disputes.
- **Service-mediated:** an app collects payment in tokens and pays its producers in tokens—users never touch cash if they already hold balance.
- **No requirement** that every trade route through a fiat conversion API.

### Reputation and quality (**Proposed**; builds on COMPUTE_REWARDS)

- Reputation from completed jobs and dispute outcomes
- Bonds that slash on proven fraud
- Service-level tags (latency, privacy, model allowlist)—filters, not one global trust theater

### Privacy tiers (**Proposed**)
...
LLM prompts usually need at least encrypted transit; full FHE for every inference is the wrong first bet (**Proposed** priority order).

---

## Pillar 3 — Additional services on the token currency (especially LLM)

*The exchange becomes useful when real products settle in it.*

### First-class service: LLM / agent gateway (**Proposed** first product)
...
**Gateway responsibilities (**Proposed**):**
...
### Marketplace for LLM jobs (**Proposed**)

| Element | Detail |
|---------|--------|
| SKU examples | “Chat completion,” “embed batch,” “agent tool loop (max N steps)” |
| Meter | Tokens in/out × model id × tier (latency / privacy) |
| Producer | Home GPU daemon, lab box, or pool operator |
| Settlement | Escrow → attest → release (Pillar 2) |
| Failover | Route to next producer if SLA miss; partial refund rules per SKU |

### How other token-priced services plug in (**Proposed**)
...
### Adjacent compute services (**Examples**)

- Encrypted storage (GB-month) paid in work tokens  
- Verification / attestation labor for identity jobs (ties back to PLANNING)  
- Relay / bandwidth for mesh apps (Pillar 4)

---

## Pillar 4 — Services defined by user payment

*Anyone can publish a paid service that settles in compute tokens.*

Illustrative verticals—**Examples**, not a commitment to build all of them. Architecture must stay extensible (**Proposed** registry pattern).

### Decentralized social media (**Example**)
...
### Car / road short-range comms (**Example**; high-risk)
...
### Todo apps / agent-powered tools (**Example**)
...
### More to come — service registry (**Proposed**)
...
User-facing “buy this feature” is just a SKU that creates one or more jobs under the hood.
...
**Relation to earlier stack (**Proposed** layering):**

| Layer | Prior docs | This proposal |
|-------|------------|---------------|
...
| Network-internal rewards | COMPUTE_REWARDS | Feeds issuance/reputation ideas; marketplace payment is consumer escrow |
| Service economy | *(this doc)* | North star for *why* tokens exist |

---

## Trust, security, and abuse

### Anti-abuse (**Proposed** directions)
...
`COMPUTE_REWARDS.md` §8 already lists Sybil, collusion, free-riding, result manipulation, resource inflation—carry those mitigations forward.

### Double-spend and settlement integrity (**Proposed**)
...
### Privacy and data (**Proposed**)

- Privacy tiers (Pillar 2); minimize metadata
...
### Centralization risk (**Proposed** mitigations)
...
Dates are order-of-work, not promises. Ponytail: smallest correct slice first.

### Phase 0 — Align and spike (now / homelab)

- This doc as **service-economy** north star
- Reconcile AGENT-CHARTER scope (network R&D vs agent-architecture side docs remain separate tracks unless merged)—open tasks already exist in `agents/user-tasks.json`
...
### Phase 1 — Work token + escrow MVP (**Proposed**)
...
### Phase 2 — LLM service gateway (**Proposed**)
...
### Phase 3 — Second service vertical (**Proposed**)
...
### Phase 4 — Harder verticals and stronger crypto (**Proposed**)
...
### Phase 5 — Scale and governance (**Proposed**)
...
1. **Substrate:** custom chain (`BLOCKCHAIN_PLATFORM.md`) vs public chain vs thin application ledger for MVP?
...
8. **Scope merge:** does this proposal supersede identity-first README marketing, or stay a parallel track? (`user-tasks.json` already asks for charter reconcile.)
9. **First spike owner:** PC lab vs potato always-on vs both?

---

## What to review next (for humans)
...
## Sources / prior notes found

Evidence from research on **2026-07-12** (PC workspace + SSH `potato`). Claims below are path-backed; absences are stated explicitly.

### PC — `agent-dump` workspace
...
| `projects/infranet/COMPUTE_REWARDS.md` | **Closest ancestor**: marketplace, rewards, reputation, tokenomics |
...
| `.cursor/rules/resource-governance.mdc` | Related practice: search/memory/model/correctness tradeoffs |
| Desktop sibling `MAIN_PROGRAMMING_FILES/Infranet/` | Source tree transferred into `projects/infranet/` (2026-06-28 ledger); same doc family still present on Desktop |
| `docs/infranet-proposal.md` / `projects/infranet/PROPOSAL.md` | Earlier same-day drafts; now point here |

### Potato (SSH `potato` — verified this session)

| Path | Notes |
|------|-------|
| `~/agent-dump/projects/infranet/*` | Same core docs as PC (README, PLANNING, COMPUTE_REWARDS, BLOCKCHAIN_PLATFORM, COST_ANALYSIS, …) |
| `~/agent-dump/projects/infranet/BASELINE.md` | **Present on potato**; source-code baseline of demo behavior. Confirms FHE/MPC/ZKP are type labels in Python, not implementations. (File notes execution was blocked on the tick that wrote it.) |
| `/media/abhinav/PERSONAL/.../ponytail-board.md` | Mentions Infranet only in a CRLF cleanup card—not product design |
| `/mnt/archive` | No additional Infranet design markdown found via `find -iname '*infranet*'` this pass |

### Laptop / USB kit

| Check | Result |
|-------|--------|
| `scripts/laptop-usb-kit/` (PC repo) | **No** Infranet / compute-token mentions (`rg`) |
| `archive/imports/from-laptop` | **Not present** in this workspace |
| Drive `E:\` | **Not mounted** this session — could not search live USB kit |
| Ledger `[LAPTOP]` product notes | No laptop-authored Infranet product brief found in searchable kit paths |

Do not invent laptop content; if a laptop clone has private notes, pull/merge later and cite then.

### Obsidian / ObsidianWriterStack

| Check | Result |
|-------|--------|
| `MAIN_PROGRAMMING_FILES/ObsidianWriterStack` | **No** matches for infranet / compute-token (`rg`) |
| Broader Obsidian vaults | Timed out / not relied on; primary sources are `projects/infranet/` |

### What was *not* found

- No prior formal “compute token as service currency + user-defined apps” proposal in `docs/infranet/` before this file.
- No laptop-authored Infranet product brief in searchable USB/kit paths this session.
- Live demos do **not** implement real FHE/MPC/ZKP or a production token (per potato `BASELINE.md` + code structure).

### Relation of this doc to older writing
